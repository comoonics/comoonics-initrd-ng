#!/usr/bin/python
"""
Integrated Management Server via normal an ssl
"""
...
try:
    from comoonics.imsd import plugins
except ImportError:
    logger.warn("Could not import imsd plugins. Limited functionality available.")
...
class Shell(cmd.Cmd):
    ...
    *****************
    ...
    INTEGRATED MANAGEMENT SERVER DEVICE (IMSD)
    """
    def __init__(self, stdin=sys.stdin, stdout=sys.stdout, user=None, passwd=None):
        ...
        self.prompt="IMSD %s<%s>$ " %(self.sysinfo.getName(), self.sysinfo.getType())
        try:
    ...
    def check_for_fence_manual(self):
        import os.path
        return os.path.exists(FENCE_MANUAL_FIFO) and os.path.exists(FENCE_MANUAL_LOCKFILE)

    def check_for_pending_fence_clients(self):
        import re
        import os
        regexp=re.compile(FENCE_CLIENT_RE)
        pending=list()
...
%s/%s""" % (PID_DIR, file)
    ...
    def do_ackmanual(self, rest):
        ...
        import os
        if self.check_for_fence_manual():
            fifo=open(FENCE_MANUAL_FIFO, "w", 0)
            logger.debug("Writing %s to fifo")
            print >>fifo, FENCE_ACK_STRING
            ...
            print >>self.stdout, "Either %s not existing or %s not existing." %(FENCE_MANUAL_FIFO, FENCE_MANUAL_LOCKFILE)
        self.lastcmd=""
    ...
    def do_restart(self, rest):
        ...
        if not os.path.exists(service_params[-1]) or os.path.splitext(service_params[-1])[1] != ".pid":
            ...
            pidfile="%s/%s.pid" %(PID_DIR, service_name)
 ...
 Pidfile defaults to %s/servicename.pid """  %(PID_DIR)
...
if __name__ == '__main__':
    ...
    try:
        ...
        comoonics.imsd.plugins.addPlugin(SysreportPlugin())
    except:
        pass
    Shell().cmdloop()
...
# - support for plugins (see comoonics.imsd.plugins)
